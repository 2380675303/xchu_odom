...
#include <xchu_mapping/xchu_odom_node.h>
...
// 初始化参数
  ...
  current_odom_pub = nh.advertise<nav_msgs::Odometry>("/current_odom", 100);

  points_sub = nh.subscribe("/filtered_points", 10, &LidarOdom::PcCB, this);
  odom_sub = nh.subscribe("/odom_raw", 50, &LidarOdom::OdomCB, this);
...
int main(int argc, char **argv) {
  ...
  LidarOdom mapping;
  //std::thread viewer(&LidarOdom::ViewerThread, &mapping);
